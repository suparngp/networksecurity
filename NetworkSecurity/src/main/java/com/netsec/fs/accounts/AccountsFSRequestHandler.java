 ...
 * @author melyssason
 */
public class AccountsFSRequestHandler extends Thread{
    Socket socket;
    DataInputStream dis;
    DataOutputStream dos;
    
    public AccountsFSRequestHandler(Socket socket){
        this.socket = socket;
    }
    ...
    public void run(){
        ...
        try{
            ...
            */
            try{
                fips = AccountsFSProvider.getFileInputStream(fileRequest);
            }
            catch(Exception e){
                e.printStackTrace();
            ...
            int blockNo = 0;
            while(moreData) {
                FileData fdata = AccountsFSProvider.getBlock(fips, blockNo);
                FileRequestResponse fileResponse = AccountsFSProvider.processFileRequest(fileRequest, fdata);
                dos.write(ReaderWriter.serialize(fileResponse));
                dos.flush();
                moreData = fdata.isMoreData();
                //AccountsFSProvider.setBlock(fops, fdata);
                blockNo++;
            }
