 ...
 * @author melyssason
 */
public class PayrollFSRequestHandler extends Thread{
    Socket socket;
    DataInputStream dis;
    DataOutputStream dos;
    
    public PayrollFSRequestHandler(Socket socket){
        this.socket = socket;
    }
    ...
    public void run(){
        ...
        try{
            ...
            PayrollFSProvider.printLog("received msg: "+ MFSchallenge);
            //Process Ticket 2 from MFS
            //send challenge response to MFS (msg 9)
            Wrapper MFSchallengeResponse = PayrollFSProvider.processMFSChallenge(MFSchallenge); 
            ...
            PayrollFSProvider.printLog("received msg: "+ MFStoFS2stream); 
            //Process Ticket 3 from MFS and process challenge response from MFS
            //Send challenge to Client. (msg 11)
            Wrapper2 UserChallenge = PayrollFSProvider.processMFStoFS2(MFStoFS2);
            ...
            Wrapper2 fsChallengeToUser = PayrollFSProvider.processUserChallengeReply(userChallengeReply);
            ...
            PayrollFSProvider.printLog("received msg: "+ fileRequest.toString());
            ...
            */
            try{
                fips = PayrollFSProvider.getFileInputStream(fileRequest);
            }
            catch(Exception e){
                e.printStackTrace();
            ...
            int blockNo = 0;
            while(moreData) {
                FileData fdata = PayrollFSProvider.getBlock(fips, blockNo);
                FileRequestResponse fileResponse = PayrollFSProvider.processFileRequest(fileRequest, fdata);
                dos.write(ReaderWriter.serialize(fileResponse));
                dos.flush();
                moreData = fdata.isMoreData();
                //PayrollFSProvider.setBlock(fops, fdata);
                blockNo++;
            }
